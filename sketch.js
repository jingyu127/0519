...
function preload() {
  // 初始化 HandPose 模型（保持 flipped: true 讓 ml5 處理鏡像）
...
function draw() {
  // 【核心修正 1】直接正常繪製影像，不要再用 translate 和 scale(-1, 1) 翻轉了！
  // 因為 createCapture({ flipped: true }) 在手機上已經把畫面翻好了。
  ...
  if (hands.length > 0) {
    ...
    if (hand.confidence > 0.3) {
      // 【核心修正 2】不要手動去用 width - x 改點位座標，直接交給 ml5 原始點位
